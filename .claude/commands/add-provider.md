...
3. Update type definitions in `packages/core/src/providers/`:
...
4. Update provider implementation in `packages/core/src/providers/model.ts`:
   - Import the provider's create function
   - Add to CREATE_AI_MAPPER
5. Update constants in `packages/extension/src/utils/constants/providers.ts`:
...
6. Add i18n translations in all locale files (packages/extension/src/locales/*.yml):
